...
func (s *OrderService) OrderDetail(ctx context.Context, request *proto.OrderRequest) (*proto.OrderInfoDetailResponse, error) {
	...
	result := global.DB.Where(&model.OrderInfo{Model: gorm.Model{ID:order.Model.ID}, User: request.UserId}).First(&order)
	if result.RowsAffected == 0 {
	...
	result = global.DB.Where(&model.OrderGoods{Order: int32(order.Model.ID)}).Find(&orderGoods)
	if result.Error != nil {
