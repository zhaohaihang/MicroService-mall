...
func ReleaseMode() {
	...
	check := &api.AgentServiceCheck{
		HTTP:                           fmt.Sprintf("http://%s:%d/order/v1/health", global.ApiConfig.Host, global.Port),
		GRPCUseTLS:                     false,
	...
	if err != nil {
		zap.S().Errorw("Error", "message", "order_api register failed", "err", err.Error())
		return
	}
	zap.S().Infow("Info", "message", "order_api register success", "port", registration.Port, "ID", ServiceID)
	...
	if err != nil {
		zap.S().Errorw("order_api service deregister failed", "err", err.Error())
		return
	}
	zap.S().Infow("order_api service deregister success", "serviceID", ServiceID)
