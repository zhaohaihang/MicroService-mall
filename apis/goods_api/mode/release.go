...
func ReleaseMode() {
	...
	if err != nil {
		zap.S().Errorw("Error", "message", "goods_api register failed", "err", err.Error())
		return
	}
	zap.S().Infow("Info", "message", "goods_api register success", "port", registration.Port, "ID", ServiceID)
	...
	if err != nil {
		zap.S().Errorw("goods_api service deregister failed", "err", err.Error())
		return
	}
	zap.S().Infow("goods_api service deregister success", "serviceID", ServiceID)
}
